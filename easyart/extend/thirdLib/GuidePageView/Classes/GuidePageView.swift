...
public class GuidePageView: UIView {
    ...
    public lazy var nextButton: UIButton = {
        let btn = UIButton.init(type: .custom)
        btn.backgroundColor = UIColor.dd.color(hexValue: 0xFF8149)
        btn.setTitleColor(UIColor.dd.color(hexValue: 0xffffff), for: .normal)
        btn.layer.cornerRadius = 5.0
        btn.layer.masksToBounds = true
        btn.setTitle("Next".localString + " >", for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 12)
        btn.titleLabel?.sizeToFit()
    ...
    private func setupSubviews(frame: CGRect) {
        ...
        for index in 0..<(imageArray?.count ?? 1) {
            ...
            //添加按钮
            nextButton.frame = CGRect.init(x: size.width - 100.0 , y: (size.height - 60.0), width: 80.0, height: 36.0)
...
//        }
        ...
        if (imageArray?.count ?? 0) == pageControl.currentPage {
            self.removeGuideViewFromSupview()
        } else {
            UIView.animate(withDuration: 0.3) {
                self.guideScrollView.contentOffset = CGPoint(x: self.guideScrollView.contentOffset.x + UIScreen.main.bounds.size.width, y: 0)
            }
        }
    }
