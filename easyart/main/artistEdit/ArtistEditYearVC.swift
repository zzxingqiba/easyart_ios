...
class ArtistEditYearVC: BaseVC {
    ...
    lazy var mTableView: UITableView = {
        ...
        tTableView.register(ArtistEditYearTableViewCell.self, forCellReuseIdentifier: "ArtistEditYearTableViewCell")
...
extension ArtistEditYearVC {
    ...
    func _loadData() {
        let model = DDSearchMulFilterModel(id: 0, title: "Year (only enter year, for example：".localString + "2020）", isSelected: self.SKUModel.year != nil && self.SKUModel.year! > 0)
        let model1 = DDSearchMulFilterModel(id: 1, title: "Unknown year".localString, isSelected: self.SKUModel.year == 0)
...
extension ArtistEditYearVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "ArtistEditYearTableViewCell") as! ArtistEditYearTableViewCell
            let model = list[indexPath.row]
            cell.selectionStyle = .none
            cell.updateUI(model: model, year: self.SKUModel.year)
            if model.isSelected {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let cell = tableView.cellForRow(at: IndexPath(row: 0, section: 0)) as? ArtistEditYearTableViewCell else { return }
        if indexPath.row == 0 {
            cell.mTextField.becomeFirstResponder()
        } else {
            self.view.endEditing(true)
            cell.mTextField.text = nil
            self.SKUModel.year = 0
            tableView.selectRow(at: indexPath, animated: false, scrollPosition: .none)
        }
    }
}
