...
class DDSearchMediumFilterVC: BaseVC {
...
extension DDSearchMediumFilterVC {
    func _bindView() {
        ...
        _ = self.mSearchFilterBottomView.clearPublish.subscribe(onNext: { [weak self] _ in
            guard let self = self else { return }
            self.searchModel.category_ids = Set<Int>()
            self._loadData()
        })
    }
    ...
    func _loadData() {
        ...
        self.list = numberList.map({ model in
            let model = DDSearchMulFilterModel(id: model["id"].intValue, title: model["title"].stringValue, isSelected: self.searchModel.category_ids.contains(where: { id in
...
extension DDSearchMediumFilterVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        ...
        if model.isSelected {
            self.searchModel.category_ids.insert(model.id)
        } else {
            self.searchModel.category_ids.remove(model.id)
        }
    }
