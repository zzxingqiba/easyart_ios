...
class SettleInUpdateCoverVC: BaseVC {
    ...
    override func createUI() {
        ...
        mTitleLabel.snp.makeConstraints { make in
            ...
            make.top.equalTo(mSettleInUpdateCoverView.snp.bottom).offset(35)
        }
    ...
    //MARK: UI
    lazy var mSettleInUpdateCoverView: SettleInUpdateCoverView = {
        let view = SettleInUpdateCoverView()
        return view
    }()
...
extension SettleInUpdateCoverVC {
    func _bindView() {
        ...
        _ = self.mConfirmButton.rx.tap.subscribe(onNext: { [weak self] _ in
            guard let self = self else { return }
            //简介
            let vc = SettleInAuthorIntroVC(profileModel: self.profileModel, model: self.model)
