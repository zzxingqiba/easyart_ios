...
class ArtistEditFrameVC: BaseVC {
...
extension ArtistEditFrameVC {
    ...
    func _loadData() {
        let model = DDSearchMulFilterModel(id: ArtistEditFrameType.include.rawValue, title: ArtistEditFrameType.include.title(), isSelected: self.SKUModel.frameType == .include)
        let model1 = DDSearchMulFilterModel(id: ArtistEditFrameType.noInclude.rawValue, title: ArtistEditFrameType.noInclude.title(), isSelected: self.SKUModel.frameType == .noInclude)
        let model2 = DDSearchMulFilterModel(id: ArtistEditFrameType.none.rawValue, title: ArtistEditFrameType.none.title(), isSelected: self.SKUModel.frameType == ArtistEditFrameType.none)
        
        self.list = [model, model1, model2]
        self.mTableView.reloadData()
    }
}

extension ArtistEditFrameVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let model = list[indexPath.row]
        self.SKUModel.frameType = ArtistEditFrameType(rawValue: model.id)
    }
}
