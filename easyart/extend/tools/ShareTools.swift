...
class ShareTools: NSObject {
    ...
    func share(type: ShareType, sourceView: UIView? = nil, complete: ((ShareResultStatus)->Void)?) {
        DispatchQueue.main.async {
            ...
            let ipadView = sourceView ?? DDUtils.shared.getCurrentVC()?.view
            ...
            DDUtils.shared.getCurrentVC()?.present(self.shareActivityVC!, animated: true, completion: nil)
        }
    }
}
